...
total_val = 0
total_test = 0
with open("/tmp/vehicle.txt", "r") as ifp:
	for line in ifp:
...
print "Total files: %d | total vehicles: " % len(unique_keys.keys()), total_vehicles, "val: %d (%0.2f%%), test: %d (%0.2f%%)" % (total_val, (total_val*100.0)/total_vehicles, total_test, (total_test*100.0)/total_vehicles,)
