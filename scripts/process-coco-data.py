...
for value in ["train", "val"]:
    ...
    with open("./coco-%s-labels.txt" % (value,), "r") as top_ifp:
        ...
        for top_line in top_ifp:
            ...
            new_label_path = "/mnt/bigdrive1/cnn/rkishore/darknet/scripts/coco-custom/labels/%s2014/" % (value,) + split_top_line[1]
            print "= New label path: %s" % (new_label_path)

            ofp = open(new_label_path, "w")

            new_image_path = "/mnt/bigdrive1/cnn/rkishore/darknet/scripts/coco-custom/images/%s2014/" % (value,) + split_top_line[1]
            new_image_path = new_image_path.replace(".txt", ".jpg")
            old_image_path = new_image_path.replace("coco-custom", "coco")
            ...
            with open(stripped_top_line, "r") as ifp:
                for line in ifp:
                    ...
                    if stripped_line.startswith("0 ") or stripped_line.startswith("2 ") or stripped_line.startswith("3 ") or \
                       stripped_line.startswith("5 ") or stripped_line.startswith("7 "):
                        ...
                        elif stripped_line.startswith("3 "):
                            writeout_line = line.replace("3 ", "1 ", 1)
                            num_motorbikes_found += 1
                            num_vehicles_found += 1
                        elif stripped_line.startswith("5 "):
                            writeout_line = line.replace("5 ", "1 ", 1)
                            num_buses_found += 1
                            num_vehicles_found += 1
                        elif stripped_line.startswith("7 "):
                            writeout_line = line.replace("7 ", "1 ", 1)
                            num_trucks_found += 1
                            num_vehicles_found += 1
    ...
    print "= STATS | %s | #persons: %d | #cars: %d | #motorbikes: %d | #buses: %d | #trucks: %d | #vehicles: %d (expected %d)" % \
