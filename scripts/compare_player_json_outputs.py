...
logging.basicConfig(filename="/mnt/bigdrive1/cnn/project1-json-check.log", format='%(asctime)s %(levelname)s:%(message)s', level=logging.INFO)
...
if __name__ == '__main__':
    ...
    for filepath in inputfiles_to_process:
        ...
        with open(filepath, "r") as ifp:
            ...
            for img_name in img_name_list:
                ...
                for objinfo in ifp_data["per_frame_info"][img_name]["regions"]:
                    cur_frame_num = int(objinfo["frame_num"])
                    if cur_frame_num not in frame_nums_seen:
                        frame_nums_seen.append(cur_frame_num)
        ...
        logging.info(" Total number of frames seen: %d" % (len(frame_nums_seen)))
        frame_nums_seen.sort(reverse=True)
        for num in frame_nums_seen:
            logging.info(" %d" % (num,))
