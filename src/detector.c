...
void prepare_detector_custom(struct prep_network_info *prep_netinfo, char *datacfg, char *cfgfile, char *weightfile)
{
  ...
  prep_netinfo->net = parse_network_cfg_custom(cfgfile, 1, 0);
  if (weightfile)
			       ...
			       struct detection_results *results_info)
{
  ...
  if (ends_with((const char *)".ts", (const char *)filename) == true)
      ...
      if (prepare_and_exec_cmd(&dummy_op_code, tmux_cmd_args, mp4_input_filename, &echild_done) < 0)
	{
      ...
      process_video(prep_netinfo, filename, mp4_input_filename, thresh, hier_thresh, NULL, outfile, 1, outjson, 0, 0, results_info);
...
void run_detector(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile);
        ...
        demo(cfg, weights, thresh, hier_thresh, cam_index, filename, names, classes, frame_skip, prefix, out_filename,
	     mjpeg_port, json_port, dont_show, ext_output, NULL);
